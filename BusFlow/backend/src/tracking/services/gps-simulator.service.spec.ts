...
import { TripStatus, BusStatus } from '@prisma/client';
    ...
    it('should track multiple buses simultaneously', async () => {
      const createMockTrip = (id: string, busId: string) => ({
        ...
        route: {
          ...
          updatedAt: new Date(),
          stops: [
            { id: `${id}-stop-1`, latitude: 28.6 + Math.random() * 0.1, longitude: 77.2 + Math.random() * 0.1, order: 1, routeId: 'route-1', createdAt: new Date(), updatedAt: new Date() },
            { id: `${id}-stop-2`, latitude: 28.7 + Math.random() * 0.1, longitude: 77.3 + Math.random() * 0.1, order: 2, routeId: 'route-1', createdAt: new Date(), updatedAt: new Date() },
          ],
        },
      });
